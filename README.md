...
Операторы `in` и `not in` (также их называют _операторы членства_) позволяют проверять,
принадлежит ли элемент определённой последовательности (строке, списку, кортежу и др.).
...
Операторы `in` и `not in` работают аналогично, проверяют наличие элемента и возвращают один из 
двух вариантов ответа - `True` или `False`.
...
Эти операторы можно использовать в ситуациях, когда необходимо проверить наличие или отсутствие 
элемента в последовательностях.
